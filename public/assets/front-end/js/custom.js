...
// Disable all recaptcha click handlers
$(".get-login-recaptcha-verify").off("click");
$(".get-login-verify-recaptcha-verify").off("click");
$(".get-regi-recaptcha-verify").off("click");
$(".get-seller-regi-recaptcha-verify").off("click");
$(".get-vendor-regi-recaptcha-verify").off("click");

// Remove functions
function customerLoginVerifyRecaptcha(url) { return; }
function customerLoginRecaptcha(url) { return; }
function customerRegiRecaptcha(url) { return; }
function sellerRegiRecaptcha(url) { return; }
function vendorRegiRecaptcha(url) { return; }
...
// Disable contact form recaptcha
$(".get-contact-recaptcha-verify").off("click");
...
function addToCartOnly(productId) {
    if (!productId) return;
    $.ajaxSetup({ headers: { "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content") } });
    $.post({
        url: "/cart/add",
        data: { id: productId, quantity: 1, product_variation_code: "", key: "", buy_now: 0 },
        beforeSend: function () { $("#loading").show(); },
        success: function (response) {
            if (response.status == 1 || response.status == 2) {
                updateNavCart();
                toastr.success(response.message || "Added to cart!", { CloseButton: true, ProgressBar: true });
            } else if (response.status == 0) {
...
function addToCart(productId, overrides = {}) {
  if (!productId) {
    ...
    id: productId,
    quantity: 1,
    product_variation_code: "", // static (override via overrides.product_variation_code)
    key: "",                    // static (override via overrides.key)
    buy_now: 1,
    ...overrides               // optional: { product_variation_code: 'XYZ', key: 169 }
  };
    ...
    function getCartSelectCartItemsRequest(checkedValues) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                toastr.success(response.message);
                cartListQuantityUpdateInit(); // Reinitialize events
...
function actionCheckoutFunctionInit() {
    $(".action-checkout-function").on("click", function () {
        ...
        if (getRoute && getRoute.toString() === "shop-cart") {
            ...
            if (typeof checkoutFromShipping === 'function') checkoutFromShipping();
        } else if (getRoute && getRoute.toString() === "checkout-payment") {
            if (typeof checkoutFromPayment === 'function') checkoutFromPayment();
        }
    });
...
// Disable auto-fill click handlers
$(".get-session-recaptcha-auto-fill").off("click");

// Disable initial auto-fill
$(".get-session-recaptcha-auto-fill").each(function () {
    // do nothing because captcha is disabled
});
