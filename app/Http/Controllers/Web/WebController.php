...
{
    // Fetch divisions for cascading dropdown
    $districts = \lemonpatwari\bangladeshgeocode\Models\District::all();

    // Cart & guest checkout validation
    if (
    ...
    // Return shipping view with all required data, including divisions
    ...
    public function shop_cart(Request $request): View|RedirectResponse
{
    if (
        ...
        $userId = auth('customer')->id() ?? session('guest_id');

        // Force all cart items to be checked
        Cart::where('customer_id', $userId)->update(['is_checked' => 1]);

        // Fetch cart again after updating is_checked
        ...
        return view(VIEW_FILE_NAMES['cart_list'], compact('topRatedShops', 'newSellers', 'currentDate', 'request', 'cart'));
    }
