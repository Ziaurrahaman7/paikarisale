...
class OrderController extends BaseController
{
    ...
    public function index(Request|null $request, $type = 'all'): View|Collection|LengthAwarePaginator|null|callable|RedirectResponse|JsonResponse
    {
        ...
        return view('admin-views.order.list', compact(
            'orders', 'searchValue', 'from', 'to', 'status', 'filter',
            'sellers', 'customer', 'vendorId', 'customerId', 'dateType'
        ));
    }
    ...
    public function exportList(Request $request, $status): BinaryFileResponse|RedirectResponse
    {
        ...
        $orders?->map(function ($order) use (&$status_array) {
    ...
    public function getView(string|int $id, DeliveryCountryCodeService $service, OrderService $orderService): View|RedirectResponse
    {
        ...
        $order = $this->orderRepo->getFirstWhere(
            params: ['id' => $id],
            relations: ['details.productAllStatus', 'verificationImages', 'shipping', 'seller.shop', 'offlinePayments', 'deliveryMan']
        );
        ...
        if (isset($order->details)) {
            foreach ($order->details as $orderDetail) {
                ...
                if ((isset($orderDetail?->product?->product_type) && $orderDetail?->product?->product_type == 'physical') ||
                    ($orderDetailProduct && isset($orderDetailProduct['product_type']) && $orderDetailProduct['product_type'] == 'physical')) {
        ...
        $totalDelivered = $this->orderRepo->getListWhere(
            filters: ['seller_id' => $order['seller_id'], 'order_status' => 'delivered', 'order_type' => 'default_type'],
            dataLimit: 'all'
        )->count();

        $shippingMethod = getWebConfig('shipping_method');
        $sellerId = ($order['seller_is'] == 'seller' && $shippingMethod == 'sellerwise_shipping') ? $order['seller_id'] : 0;

        $filters = ['is_active' => 1, 'seller_id' => $sellerId];
        ...
        if ($order['order_type'] == 'default_type') {
            ...
            return view(Order::VIEW[VIEW], compact(
                'order', 'linkedOrders', 'deliveryMen', 'totalDelivered', 'companyName', 'companyWebLogo',
                'physicalProduct', 'countryRestrictStatus', 'zipRestrictStatus', 'countries', 'zipCodes', 'orderCount', 'isOrderOnlyDigital'
            ));
        } else {
    ...
    public function generateInvoice(string|int $id)
    {
        ...
        $mpdfView = PdfView::make('admin-views.order.invoice', compact(
            'order', 'vendor', 'companyPhone', 'companyEmail', 'companyName', 'companyWebLogo', 'invoiceSettings'
        ));
