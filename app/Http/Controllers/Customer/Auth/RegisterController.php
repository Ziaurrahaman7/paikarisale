...
use lemonpatwari\bangladeshgeocode\Models\Upazila;
    ...
    public function getRegisterView(): View
{
    ...
    $districts = District::all(); // fetch all divisions

    return view(
        'web-views.customer-views.auth.register',
        compact('recaptcha', 'districts')
    );
}

public function submitRegisterData(CustomerRegistrationRequest $request): JsonResponse|RedirectResponse
{
    $referUser = $request['referral_code'] 
        ? $this->customerRepo->getFirstWhere(params: ['referral_code' => $request['referral_code']]) 
        : null;
    ...
    // Prepare registration data (no division, no last name, no email)
    $registerData = [
    ...
    // Create customer
    $user = $this->customerRepo->add(data: $registerData);
    ...
    // Only phone verification (email removed)
    ...
    if ($request->ajax()) {
        if ($phoneVerification && !$user->is_phone_verified) {
            ...
            return response()->json([
                'redirect_url' => route('customer.auth.check-verification', [
