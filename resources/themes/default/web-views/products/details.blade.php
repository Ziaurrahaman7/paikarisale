        ...
        @if($product->images!=null && json_decode($product->images)>0)
            @if(json_decode($product->colors) && count($product->color_images_full_url)>0)
                @foreach ($product->color_images_full_url as $key => $photo)
                    @php
                        $isColor = $photo['color'] != null;
                        $itemId = $isColor ? 'image'.$photo['color'] : 'image'.$key;
                        $imageSrc = getStorageImages(path: $photo['image_name'], type: 'product');
                    @endphp
                    <div class="product-preview-item d-flex align-items-center justify-content-center {{ $key==0?'active':''}}" id="{{ $itemId }}">
                        <img class="img-responsive w-100 no-zoom" 
                             src="{{ $imageSrc }}" 
            ...
            @else
                @foreach ($product->images_full_url as $key => $photo)
                    @php $imageSrc = getStorageImages($photo, type: 'product'); @endphp
...
<style>
    /* Disable zoom effect on all product images in the preview */
    .no-zoom {
                                    ...
                                    <div class="__btn-grp mt-2 mb-3 product-add-and-buy-section-parent">
                                        ...
                                        <div
                                            ...
                                            @else
                                                ...
                                                >
                                                    <i class="czi-card"></i> {{ translate('buy_now') }}
                                                </button>
                                                <button
                                                    class="btn btn--primary element-center product-add-to-cart-button d-none"
                                                    type="button"
                                                    data-form=".add-to-cart-details-form"
